...
define([
        ...
        "ubuild/security/team/MemberSelector",
        "ubuild/rightPanel/RightPanelUsersAndGroups"
    ],
function(
    _TemplatedMixin,
    _Widget,
    ...
    return declare([_Widget, _TemplatedMixin], {
        ...
        showForm: function() {
            ...
            if (!_this.user.isSecurityAdmin || !_this.user.isTeamUserManager){
                domClass.add(_this.domNode, "edit-team-read-only");
            }

            this.form = new ColumnForm({
                submitUrl: bootstrap.restUrl + "/team" + (_this.team ? "/" + _this.team.id : ""),
                submitMethod: _this.team ? "PUT" : "POST",
                cancelLabel: null,
                showButtons: _this.user.isSecurityAdmin,
                ...
                postSubmit: function(data) {
                    if (_this.callback !== undefined) {
                        if (_this.team) {
                            ...
                            _this.callback(true, !_this.rightPanel.panelHidden);
                        }
                        else {
                            _this.callback(true, data.id);
                        }
                    }
                },
                onCancel: function() {
            ...
            this.form.addField({
                name: "name",
                label: i18n("Name"),
                required: _this.user.isSecurityAdmin,
                type: "Text",
                readOnly: !_this.user.isSecurityAdmin,
