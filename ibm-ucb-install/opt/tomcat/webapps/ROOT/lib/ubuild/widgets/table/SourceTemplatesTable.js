...
function(
     ...
     *  tasksUrl / String                   The URL for the SourceConfigTemplateTasks class. Required.
     */

    return declare('js.ubuild.widgets.table.SourceTemplatesTable', [_Widget, _TemplatedMixin], {
        templateString:
            '<div class="sourceTemplatesTable">'+
                '<div data-dojo-attach-point="sourceTemplatesTableAttachPoint"></div>'+
            '</div>',
        tasksUrl: undefined,
        ...
        postCreate: function() {
            ...
            var gridRestUrl = bootstrap.restUrl + "/sourceTemplates";
            var gridLayout = [{
                    name: i18n("Name"),
                    field: "name",
                    orderField: "name",
            ...
            this.grid.placeAt(this.sourceTemplatesTableAttachPoint);
        },
        ...
        viewTemplate: function(target) {
            var self = this;
            goTo(self.tasksUrl + "/viewSourceConfigTemplate?sourceConfigTemplateId=" + target.id);
        },

        viewUsage: function(target) {
            var self = this;
            showPopup(self.tasksUrl + "/usageSourceConfigTemplate?sourceConfigTemplateId=" + target.id, 800, 400);
        },

        copyTemplate: function(target) {
            var self = this;
            showPopup(self.tasksUrl + "/copySourceConfigTemplate?sourceConfigTemplateId=" + target.id, 800, 400);
        },
        ...
        createEditLink: function(parent, item) {
            ...
            viewLink.href = self.tasksUrl + "/viewSourceConfigTemplate?sourceConfigTemplateId=" + item.id;
        ...
        confirmDelete: function(target) {
            ...
            var confirm = new GenericConfirm({
                ...
                action: function() {
                    self.grid.block();
                    xhr.del({
                        url: bootstrap.restUrl + "/sourceTemplates/" + target.id,
                        handleAs: "json",
                        load: function(data) {
                            self.grid.unblock();
                            self.grid.refresh();
