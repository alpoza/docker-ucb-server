...
<%@attribute name="valueType"    required="false" type="java.lang.String" %><%-- The value type to submit. Accepts 'id' (default), 'name', and 'string'--%>
    ...
    document.observe('dom:loaded', function(event) { // no need to put in the header, no JS i18n used
        ...
        var values = [
            <c:forEach var="element" items="${list}" varStatus="loop">
              <c:choose>
                <c:when test="${valueType eq 'string'}">
                  { 'name': '${fn:escapeXml(element)}',
                    'id': '${element}'
                </c:when>
                <c:otherwise>
                  { 'name': '${fn:escapeXml(element.name)}',
                    'id':
                    <c:if test="${valueType eq 'id'}">'${element.id}'</c:if>
                    <c:if test="${valueType eq 'name'}">'${element.name}'</c:if>
                </c:otherwise>
              </c:choose>
  ...
  <div style="width: ${size}em; padding-bottom: 2em;">
    <c:choose>
      <c:when test="${not empty selectedText}">
        <ucf:text name="auto-complete-${autoCompleteId}" value="${selectedText}" size="${size}" enabled="${enabled}"/>
      </c:when>
      <c:otherwise>
        <c:if test="${selectedObject == null and selectedId != null}">
          <c:forEach var="item" items="${list}">
            <c:if test="${item.id == selectedId}"><c:set var="selectedObject" value="${item}" scope="page"/></c:if>
          </c:forEach>
        </c:if>

        <c:choose>
          <c:when test="${valueType eq 'string'}">
            <ucf:text name="auto-complete-${autoCompleteId}" value="${selectedObject == null ? emptyMessage : selectedObject}" size="${size}" enabled="${enabled}"/>
          </c:when>
          <c:otherwise>
  ...
  <c:choose>
      <c:when test="${canUnselect && (! empty selectedList)}">
          <c:forEach var="selected" items="${selectedList}">
            <c:if test="${valueType eq 'id'}"><c:set var="selectedItemId" value="${selected.id}"/></c:if>
            <c:if test="${valueType eq 'name'}"><c:set var="selectedItemId" value="${selected.name}"/></c:if>
            <c:if test="${valueType eq 'string'}"><c:set var="selectedItemId" value="${selected}"/></c:if>
            <input <c:if test="${!empty id}">id="${fn:escapeXml(id)}" </c:if> name="${fn:escapeXml(name)}" value="${selectedItemId}" type="hidden"/>
          </c:forEach>
      </c:when>
      <c:otherwise>
          <c:if test="${valueType eq 'id'}"><c:set var="selectedItemId" value="${selectedObject.id}"/></c:if>
          <c:if test="${valueType eq 'name'}"><c:set var="selectedItemId" value="${selectedObject.name}"/></c:if>
          <c:if test="${valueType eq 'string'}"><c:set var="selectedItemId" value="${selectedObject}"/></c:if>
          <input <c:if test="${!empty id}">id="${fn:escapeXml(id)}" </c:if> name="${fn:escapeXml(name)}" value="${selectedItemId}" type="hidden"/>
      </c:otherwise>
  </c:choose>
</div>
