...
<c:url var="uploadFileUrl" value="${CodestationBuildLifeTasks.uploadFile}"/>
    ...
    <table class="property-table">
        ...
        <tbody>
            ...
            <tr class="${fn:escapeXml(eo.last)}" valign="top">
                <td><span class="bold">${ub:i18n("ArtifactSetWithColon")} <span class="required-text">*</span></span></td>
                <td>
                    <ucf:stringSelector name="${WebConstants.CODESTATION_ARTIFACT_SET}"
                        list="${setList}"/>
                </td>
                <td>
                    <span class="inlinehelp">${ub:i18n("CodeStationBuildLifeArtifactSetAddFileDesc")}</span>
                </td>
            </tr>
            
            <error:field-error field="subdir" cssClass="${eo.next}"/>
            <tr class="${fn:escapeXml(eo.last)}" valign="top">
                <td><span class="bold">${ub:i18n("DirectoryWithColon")}</span></td>
                <td>
                    <ucf:text name="subdir" value=""/>
                </td>
                <td>
                    <span class="inlinehelp">${ub:i18n("CodeStationBuildLifeArtifactSetDirectoryDesc")}</span>
                </td>
            </tr>
            
            <error:field-error field="file" cssClass="${eo.next}"/>
            <tr class="${fn:escapeXml(eo.last)}" valign="top">
                <td><span class="bold">${ub:i18n("FileWithColon")} <span class="required-text">*</span></span></td>
                <td>
                    <input type="file" name="file">
                </td>
                <td>
                    <span class="inlinehelp">${ub:i18n("CodeStationBuildLifeArtifactSetFileDesc")}</span>
                </td>
            </tr>
            
            <tr>
                <td colspan="3">
                    <ucf:button name="UploadFile" label="${ub:i18n('UploadFile')}" enabled="true"/>
