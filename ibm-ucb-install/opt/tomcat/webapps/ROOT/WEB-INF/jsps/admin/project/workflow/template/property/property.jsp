...
<ah3:useTasks id="WorkflowTemplateTasksNoConversation" class="com.urbancode.ubuild.web.admin.project.workflow.template.WorkflowTemplateTasks" useConversation="false" />

<c:set var="inEditMode" value="${fn:escapeXml(mode) == 'edit'}" />
<c:set var="inViewMode" value="${!inEditMode}" />

<c:url var="saveUrl" value="${WorkflowTemplateTasks.saveProperty}" />
<c:url var="cancelUrl" value="${WorkflowTemplateTasks.cancelProperty}">
  <c:if test="${param.workflowTemplatePropSeq != null}">
    <c:param name="workflowTemplatePropSeq" value="${param.workflowTemplatePropSeq}" />
  </c:if>
</c:url>

<c:url var="editUrl" value="${WorkflowTemplateTasksNoConversation.editProperty}">
...
<c:url var="viewUrl" value="${WorkflowTemplateTasks.viewProperty}">
  <c:param name="${WebConstants.WORKFLOW_TEMPLATE_ID}" value="${workflowTemplate.id}" />
</c:url>

<c:url var="viewListUrl" value="${WorkflowTemplateTasks.viewPropertyList}">
  <c:param name="${WebConstants.WORKFLOW_TEMPLATE_ID}" value="${workflowTemplate.id}" />
</c:url>
...
<%
    ...
    WorkflowTemplateProperty workflowTemplateProp = (WorkflowTemplateProperty) pageContext.findAttribute("workflowTemplateProp");
    if (workflowTemplateProp != null) {
        pageContext.setAttribute("inputPropString", StringUtils.join(workflowTemplateProp.getInputProperties(), "\n"));
        pageContext.setAttribute("agentFilter", workflowTemplateProp.getJobExecutionAgentFilter());

        pageContext.setAttribute("isDefinedProp",workflowTemplateProp.isDefinedValue());
        }
%>
...
<script type="text/javascript">
    ...
    function updateFieldDisplay() {
        ...
        <c:if test="${workflowTemplate.originating}">
        if ($('isUserMayOverride').checked && propertyTypeId !== textSecureType) {
            var saveToBuildConfig = $('saveToBuildConfigField');
            saveToBuildConfig.show();
            saveToBuildConfig.checked = "${param['saveToBuildConfig'] != null ? param['saveToBuildConfig'] : workflowTemplateProp.saveToBuildConfig}";
        }
        else {
            $('saveToBuildConfigField').hide();
        }
    ...
    function initPluginList() {
        ...
        var selectedValue = "${ah3:escapeJs(param['propertyGroupList'] != null ? param['propertyGroupList'] : workflowTemplateProp.displayValue)}";
...
<div class="tab-content">
  ...
  <form method="post" action="${fn:escapeXml(saveUrl)}#property">
    <c:if test="${param.workflowTemplatePropSeq != null}">
      <input type="hidden" name="${WebConstants.WORKFLOW_TEMPLATE_PROP_SEQ}"
        value="${fn:escapeXml(param.workflowTemplatePropSeq)}" />
    </c:if>

    <table class="property-table">
      <tbody>
        <c:set var="fieldName" value="${WebConstants.NAME}"/>
        <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.name}"/>
        ...
        <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.label}"/>
        ...
        <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.description}"/>
        ...
        <c:set var="value" value="${not empty param[fieldName] ? param[fieldName] : workflowTemplateProp.userMayOverride}"/>
        ...
        <c:if test="${workflowTemplate.originating}">
          <tbody id="saveToBuildConfigField">
            ...
            <c:set var="value" value="${not empty param[fieldName] ? param[fieldName] : workflowTemplateProp.saveToBuildConfig}"/>
        ...
        <c:set var="value" value="${not empty param[fieldName] ? param[fieldName] : workflowTemplateProp.interfaceType.id}"/>
        ...
        <tr valign="top" class="${eo.last}">
          ...
          <tbody id="pluginTypeFields">
            ...
            <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.pluginType}"/>
            <tr class="${fn:escapeXml(eo.last)}">
          ...
          <td colspan="2" style="border-bottom: 0px;">
            <div id="definedValueDiv"
              ...
              <c:choose>
                ...
                <c:otherwise>
                  <c:if test="${!workflowTemplateProp.scriptedValue}">
                    <c:set var="defined" value="true"/>
                  </c:if>
                  <c:if test="${workflowTemplateProp.scriptedValue}">
                    <c:set var="scripted" value="true"/>
                  </c:if>
                  <c:if test="${workflowTemplateProp.jobExecutionValue}">
                    <c:set var="job" value="true"/>
                  </c:if>
                </c:otherwise>
              </c:choose>
      ...
      <tbody id="valueRequiredFields" ${workflowTemplateProp.interfaceType.checkbox ? 'style="display:none;"' : ''}>
        <c:set var="fieldName" value="isRequired"/>
        <c:set var="value" value="${not empty param[fieldName] ? param[fieldName] : workflowTemplateProp.required}"/>
      ...
      <c:set var="isSelect"
        value="${workflowTemplateProp.interfaceType.select or workflowTemplateProp.interfaceType.multiSelect}" />

      <c:set var="hideAllowedValues"
        value="${!isSelect or workflowTemplateProp.scriptedValue or workflowTemplateProp.jobExecutionValue}" />

      <tbody id="allowedValuesFields" <c:if test="${hideAllowedValues}">style="display:none;"</c:if>>
        <c:set var="fieldName" value="allowedValues"/>
        <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.allowedValues}"/>
      ...
      <c:set var="hideAllowedValuesScript" value="${!isSelect and not workflowTemplateProp.scriptedValue}" />
      ...
      <tbody id="allowedValuesScriptFields" <c:if test="${hideAllowedValuesScript}">style="display:none;"</c:if>>
        ...
        <tr class="${fn:escapeXml(eo.last)}">
          ...
          <td colspan="2" style="border-top: 0px;"><c:remove var="allowedValuesString" /> <c:forEach var="allowedValue"
            ...
            <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.allowedValuesScript}"/>
      ...
      <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.displayValue}"/>
      <tbody id="checkboxFields" ${workflowTemplateProp.interfaceType.checkbox ? '' : 'style="display: none;"'}>
        ...
        <c:if test="${(not empty workflowTemplateProp) && isDefinedProp}">
          <tr class="${fn:escapeXml(eo.last)}">
      ...
      <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.displayValue}"/>
      <tbody id="textAreaFields" ${workflowTemplateProp.interfaceType.textArea ? '' : 'style="display: none;"'}>
        <error:field-error field="value" cssClass="${eo.last}" />
        <tr class="${fn:escapeXml(eo.last)}">
          <td width="15%" style="border-top: 0px;"><span class="bold" style="margin-left: 25px;">${ub:i18n("DefaultValueWithColon")}</span></td>
      ...
      <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.displayValue}"/>
      <tbody id="textSecureFields" ${workflowTemplateProp.interfaceType.textSecure ? '' : 'style="display: none;"'}>
        <error:field-error field="value" cssClass="${eo.last}" />
        <tr class="${fn:escapeXml(eo.last)}">
          <td width="15%" style="border-top: 0px;"><span class="bold" style="margin-left: 25px;">${ub:i18n("DefaultValueWithColon")}</span></td>
          ...
          <td style="border-top: 0px;"><span class="inlinehelp">T${ub:i18n("PropertyDefaultValueDesc")}</span></td>
        </tr>
        <c:if test="${(not empty workflowTemplateProp) && isDefinedProp}">
          <tr class="${fn:escapeXml(eo.last)}">
            <td width="15%" style="border-top: 0px;"><span class="bold" style="margin-left: 25px;">${ub:i18n("PropertyChangeExistingValuesWithColon")}</span></td>
      ...
      <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.displayValue}"/>
      <tbody id="textAndSelectFields" ${workflowTemplateProp.interfaceType.text || workflowTemplateProp.interfaceType.select ? '' : 'style="display: none;"'}>
        <error:field-error field="value" cssClass="${eo.last}" />
        <tr class="${fn:escapeXml(eo.last)}">
          <td width="15%" style="border-top: 0px;"><span class="bold" style="margin-left: 25px;">${ub:i18n("DefaultValueWithColon")}</span></td>
      ...
      <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.displayValue}"/>
      <tbody id="multiSelectFields" ${workflowTemplateProp.interfaceType.multiSelect ? '' : 'style="display: none;"'}>
        <error:field-error field="value" cssClass="${eo.last}" />
        <tr class="${fn:escapeXml(eo.last)}">
          <td width="15%" style="border-top: 0px;"><span class="bold" style="margin-left: 25px;">${ub:i18n("DefaultValueWithColon")}</span></td>
      ...
      <tbody id="propertyGroupFields" ${workflowTemplateProp.interfaceType.integrationPlugin ? '' : 'style="display:none;"'}>
      ...
      <tbody id="scriptedValueFields" ${workflowTemplateProp.scriptedValue ? '' : 'style="display: none;"'}>
        ...
        <tr class="${fn:escapeXml(eo.last)}">
          ...
          <td colspan="2" style="border-top: 0px;">
            ...
            <%-- allow for props of type: text, textArea select, multi-select  --%> <c:if
              test="${workflowTemplateProp.interfaceType.multiValued || workflowTemplateProp.interfaceType.text || workflowTemplateProp.interfaceType.textArea || workflowTemplateProp.interfaceType.checkbox}">
              <%--
              ...
              <span class="bold">${ub:i18n("PropertyBeanshellScript")} <c:if
                  test="${not workflowTemplateProp.interfaceType.multiValued}">
            ...
            <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.valueScript}"/>
      ...
      <tbody id="jobValueFields" ${workflowTemplateProp.jobExecutionValue ? '' : 'style="display: none;"'}>
        ...
        <tr class="${fn:escapeXml(eo.last)}">
          ...
          <c:set var="value" value="${param[fieldName] != null ? param[fieldName] : workflowTemplateProp.jobExecutionJobConfig.id}"/>
          <td style="border-top: 0px;">
            <ucf:idSelector
