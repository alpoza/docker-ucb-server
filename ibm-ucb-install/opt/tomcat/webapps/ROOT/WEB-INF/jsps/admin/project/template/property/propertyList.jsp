...
<c:url var="movePropertyUrl" value='<%= new ProjectTemplateTasks().methodUrl("moveProperty", false)%>'>
  <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
</c:url>
<c:url var="pasteFromClipboardUrl" value='${ProjectTemplateTasks.pasteFromClipboard}'>
  <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
</c:url>
<c:url var="pasteAndRemoveFromClipboardUrl" value='${ProjectTemplateTasks.pasteAndRemoveFromClipboard}'>
  <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
</c:url>
...
<script type="text/javascript"> /* <![CDATA[ */
    ...
    var moveProperty = function(fromIdx, toIdx) {
        var url = ${ah3:toJson(movePropertyUrl)}+
            '&'+ WebConstants.PROJECT_TEMPLATE_PROP_SEQ+'='+fromIdx+
            "&"+ "to"+WebConstants.PROJECT_TEMPLATE_PROP_SEQ+'='+toIdx;
        if (fromIdx === toIdx) {
            // TODO
...
<div class="component-heading">
  <div style="float: right;">
    <c:url var="newPropertyUrl" value="${ProjectTemplateTasks.newProperty}">
      <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
    </c:url>
...
<table id="PropertyTable" class="data-table">
  ...
  <tbody>
    <c:choose>
      <c:when test="${empty projectTemplate.propertyList}">
        <tr bgcolor="#ffffff">
          <td colspan="6">${ub:i18n("PropertyNoneDefinedMessage")}</td>
        </tr>
      </c:when>
      <c:otherwise>
        <c:forEach var="property" items="${projectTemplate.propertyList}" varStatus="status">
          <c:url var="editPropertyUrlSeq" value="${ProjectTemplateTasks.editProperty}">
            <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
            <c:param name="${WebConstants.PROJECT_TEMPLATE_PROP_SEQ}" value="${status.index}"/>
          </c:url>

          <c:url var="copyPropertyToClipboardUrl" value='${ProjectTemplateTasks.copyPropertyToClipboard}'>
            <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
            <c:param name="${WebConstants.PROJECT_TEMPLATE_PROP_SEQ}" value="${status.index}"/>
          </c:url>

          <c:url var="removePropertyUrlSeq" value='<%= new ProjectTemplateTasks().methodUrl("removeProperty", false)%>'>
            <c:param name="${WebConstants.PROJECT_TEMPLATE_ID}" value="${projectTemplate.id}"/>
            <c:param name="${WebConstants.PROJECT_TEMPLATE_PROP_SEQ}" value="${status.index}"/>
          </c:url>
          ...
          <tr id="${rowId}" class="property-row">
            <td nowrap="nowrap">
              <c:if test="${workflowProp==null && canEdit}">
                <script type="text/javascript"> /* <![CDATA[ */
              ...
              <ucf:link href="${editPropertyUrlSeq}#property" label="${property.name}" enabled="${workflowProp==null && canEdit}"/>
            </td>
            ...
            <td>
              <%
                ProjectTemplateProperty property = (ProjectTemplateProperty) pageContext.getAttribute("property");
                ...
                if (property.getInterfaceType().equals(PropertyInterfaceTypeEnum.CHECKBOX)) {
                    if (!StringUtils.isEmpty(value)) {
                        pageContext.setAttribute("propertyValue", TranslateMessage.translate(value));
                    }
                }
                else {
            ...
            <td align="center" height="1" nowrap="nowrap">
              <ucf:link href="${editPropertyUrlSeq}#property" img="${iconPencilUrl}" label="${ub:i18n('Edit')}"
                enabled="${canEdit}"/> &nbsp;
              <ucf:link href="${copyPropertyToClipboardUrl}"
                label="${ub:i18n('CopyVerb')}" title="${ub:i18n('CopyVerb')}" img="${iconCopyUrl}" enabled="${canEdit}"/>&nbsp;
              <ucf:deletelink href="${removePropertyUrlSeq}" name="${property.name}" label="${ub:i18n('Remove')}"
                img="${iconDeleteUrl}" enabled="${canEdit}"/>
            </td>
          </tr>
        </c:forEach>
      </c:otherwise>
