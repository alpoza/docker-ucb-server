...
<%-- *** CONTENT *** --%>
<br/>
<form method="post" action="${saveUrl}#trigger">
  ...
  <table class="property-table">
    <tbody>
      ...
      <error:field-error field="forced" cssClass="<%=eo.getNext() %>"/>
      <tr class="<%=eo.getLast() %>">
        <td align="left" width="20%"><span class="bold">${ub:i18n("Force")} </span></td>
        <td align="left">
          <ucf:checkbox name="forced"
                          value="true"
                          checked="${trigger.forced}"
                          enabled="${inEditMode}"/>
        </td>
        <td>
          <span class="inlinehelp">${ub:i18n("TriggerForceDesc")}</span>
        </td>
      </tr>
