...
<c:url var="movePropertyUrl" value='<%= new WorkflowTasks().methodUrl("moveProperty", false)%>'>
...
<script type="text/javascript"> /* <![CDATA[ */
    ...
    var moveProperty = function(fromIdx, toIdx) {
        var url = ${ah3:toJson(movePropertyUrl)}+
            '&'+ WebConstants.WORKFLOW_PROP_SEQ+'='+fromIdx+
            "&"+ "to"+WebConstants.WORKFLOW_PROP_SEQ+'='+toIdx;
        if (fromIdx === toIdx) {
            // TODO
...
<table id="PropertyTable" class="data-table">
  <thead>
    <tr>
      ...
      <th scope="col"            >${ub:i18n("BuildConfigValueDefault")}</th>
  ...
  <tbody>
    <c:choose>
      <c:when test="${empty workflow.propertyList}">
        <tr bgcolor="#ffffff">
          <td colspan="8">${ub:i18n("PropertyNoPropertiesMessage")}</td>
        </tr>
      </c:when>
      <c:otherwise>
        <c:forEach var="property" items="${workflow.propertyList}" varStatus="status">
          <c:url var="editPropertyUrlSeq" value="${WorkflowTasks.editProperty}">
            <c:param name="${WebConstants.WORKFLOW_ID}" value="${workflow.id}"/>
            <c:param name="${WebConstants.WORKFLOW_PROP_SEQ}" value="${status.index}"/>
          </c:url>

          <c:url var="removePropertyUrlSeq" value='<%= new WorkflowTasks().methodUrl("removeProperty", false)%>'>
            <c:param name="${WebConstants.WORKFLOW_ID}" value="${workflow.id}"/>
            <c:param name="${WebConstants.WORKFLOW_PROP_SEQ}" value="${status.index}"/>
          </c:url>
          ...
          <tr id="${rowId}" class="property-row">
            ...
            <td>
              <%
                WorkflowProperty property = (WorkflowProperty) pageContext.getAttribute("property");
                ...
                if (property.getInterfaceType().isIntegrationPlugin() && !StringUtils.isBlank(value)) {
                    ...
                    if (propSheet != null) {
                        ...
                        value = "Invalid Property Group Selected";
                    }
                }
            ...
            <td align="center" height="1" nowrap="nowrap">
              <ucf:link href="${editPropertyUrlSeq}#property" img="${iconPencilUrl}" label="${ub:i18n('Edit')}" enabled="${workflowProp==null && canEdit}"/> &nbsp;
              <ucf:deletelink href="${removePropertyUrlSeq}" name="${property.name}" label="${ub:i18n('Remove')}" img="${iconDeleteUrl}" enabled="${workflowProp==null && canEdit}"/>
            </td>
          </tr>
        </c:forEach>
      </c:otherwise>
