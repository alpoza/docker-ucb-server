...
<c:url var="auditUrl" value="${ProjectTasks.queryAudit}">
  <c:param name="${WebConstants.PROJECT_ID}" value="${project.id}"/>
</c:url>

<c:url var="exportUrl" value="${ProjectTasks.exportCSV}">
...
<jsp:include page="/WEB-INF/jsps/home/project/header.jsp">
  ...
  <jsp:param name="selected" value="audit"/>
...
%>
                  <c:url var="viewPageUrl" value="${ProjectTasks.queryAudit}">
                      <c:param name="${WebConstants.USER_ID}" value="${param.userId}"/>
                      <c:param name="${WebConstants.START_TIME}" value="${param.startTime}"/>
                      <c:param name="${WebConstants.END_TIME}" value="${param.endTime}"/>
                      <c:param name="${WebConstants.PROJECT_ID}" value="${project.id}"/>
                    ...
                    <table class="data-table" width="100%">
                      ...
                      <c:forEach var="queryResult" items="${queryResults}" varStatus="uowStatus">
                          ...
                          <tr class="${eo.next}">
                            ...
                            <td nowrap="nowrap">${fn:escapeXml(queryResult.action)}</td>
                            <td nowrap="nowrap">${fn:escapeXml(queryResult.objectType)}</td>
