...
<jsp:include page="/WEB-INF/jsps/admin/config/mainConfigurationTabsHeader.jsp">
  <jsp:param name="title" value="${ub:i18n('ProcessTemplates')}"/>
  <jsp:param name="selected" value="Process"/>
</jsp:include>

<c:url var="templateTasksUrl" value="/tasks/admin/project/workflow/template/WorkflowTemplateTasks"/>
        ...
        <div class="component-heading">
          <div style="float: right;">
            <c:if test="${canCreateTemplates}">
              <c:url var="createTemplateUrl" value="${WorkflowTemplateTasks.newWorkflowTemplate}"/>
          ...
          ${ub:i18n("ProcessTemplates")}
        </div>
        <div id="processTemplateList"></div>
        <script type="text/javascript">
            require(["ubuild/module/UBuildApp", "ubuild/table/ProcessTemplatesTable"], function(UBuildApp, ProcessTemplatesTable) {
                UBuildApp.util.i18nLoaded.then(function() {
                    var table = new ProcessTemplatesTable({ "tasksUrl": "${templateTasksUrl}" });
                    table.placeAt("processTemplateList");
                });
            });
        </script>
        <br/>
