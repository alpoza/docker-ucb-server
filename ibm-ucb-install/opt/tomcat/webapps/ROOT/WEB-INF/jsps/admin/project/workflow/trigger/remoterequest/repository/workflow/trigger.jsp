...
<form method="post" action="${fn:escapeXml(saveUrl)}#trigger">
  ...
  <span class="bold">${ub:i18n("RepositoryRequestTrigger")}</span>
      ...
      <tr class="<%=eo.getLast() %>">
        ...
        <td align="left" colspan="2">
          <span class="inlinehelp">${ub:i18n("TriggerBuildLifeScriptDesc")}<br/>
            &nbsp;&nbsp;&nbsp;${ub:i18nMessage("TriggerBuildLifeScriptDescExample", fn:escapeXml(trigger.workflow.project.name))}<br/>
            &nbsp;&nbsp;&nbsp;${ub:i18nMessage("TriggerBuildLifeScriptDescWorkflow", fn:escapeXml(trigger.workflow.name))}<br/>
            &nbsp;&nbsp;&nbsp;${ub:i18n("TriggerBuildLifeScriptDescProperties")}<br/>
          <br/>
          ${ub:i18n("TriggerBuildLifeScriptDescExample")}<br/>
            <br/>
            <span class="code">import com.urbancode.ubuild.domain.buildlife.*;<br/>
return BuildLifeFactory.getInstance().restoreMostRecentForProfile(project.getWorkflow("Originating Workflow").getBuildProfile());</span>
            <br/>
          </span><br/>
      ...
      <c:if test="${inViewMode}">
        <tr class="<%=eo.getNext() %>">
          <td align="left" width="20%"><span class="bold">${ub:i18n("TriggerURL")} </span></td>
          <td align="left" colspan="2">
            <c:url var="triggerUrl" value="/trigger">
              <c:param name="code" value="${trigger.code}"/>
            </c:url>
            <span class="inlinehelp">${ub:i18n("TriggerURLDesc")}</span><br/><br/>
            ${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${triggerUrl}
          </td>
        </tr>
      </c:if>
