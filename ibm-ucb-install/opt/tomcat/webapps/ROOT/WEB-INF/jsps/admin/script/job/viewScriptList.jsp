...
<%@page import="com.urbancode.ubuild.domain.script.job.*"%>
<%@page import="com.urbancode.ubuild.domain.security.UBuildAction"%>
<%@page import="com.urbancode.ubuild.domain.security.Authority"%>
<%@page import="com.urbancode.ubuild.web.admin.script.job.JobPreConditionScriptTasks"%>
...
<c:url var="newUrl"    value='<%=new JobPreConditionScriptTasks().methodUrl("newScript", false)%>'/>
<c:url var="deleteUrl" value='<%=new JobPreConditionScriptTasks().methodUrl("deleteScript", false)%>'/>
<c:url var="doneUrl" value="${SystemTasks.viewIndex}"/>

<%
  JobPreConditionScriptFactory factory = JobPreConditionScriptFactory.getInstance();
...
<div>
    <div class="tabManager" id="secondLevelTabs">
      <ucf:link label="${ub:i18n('JobPreConditionScripts')}" href="" enabled="${false}" klass="selected tab"/>
    </div>
    <div class="contents">
        <div class="system-helpbox">
            ${ub:i18n("JobScriptSystemHelpBox")}
      </div>
          ...
          <tbody>
            ...
            <c:choose>
              <c:when test="${empty scriptList}">
                <tr bgcolor="#ffffff">
                  <td colspan="4">${ub:i18n("JobScriptNoneConfiguredMessage")}</td>
                </tr>
              </c:when>
              <c:otherwise>
                  ...
                  <%
                    JobPreConditionScript script = (JobPreConditionScript) pageContext.findAttribute("script");
                    String[] projectNameList = factory.getActiveProjectNamesForJobPreConditionScript(script);
                    pageContext.setAttribute("projectNameList", projectNameList);

                    String[] libWorkflowDefNameList = factory.getLibararyWorkflowDefinitionNamesForJobPreConditionScript(script);
                    pageContext.setAttribute("libWorkflowDefNameList", libWorkflowDefNameList);
                  %>

                  <c:url var='viewUrlId' value='<%=new JobPreConditionScriptTasks().methodUrl("editScript", false)%>'>
                    <c:param name="${WebConstants.SCRIPT_ID}" value="${script.id}"/>
                  </c:url>

                  <c:url var="copyScriptUrl" value="${JobPreConditionScriptTasks.copyScript}">
                    <c:param name="${WebConstants.SCRIPT_ID}" value="${script.id}"/>
                  </c:url>

                  <c:url var='deleteUrlId' value='<%=new JobPreConditionScriptTasks().methodUrl("deleteScript", false)%>'>
                    <c:param name="${WebConstants.SCRIPT_ID}" value="${script.id}"/>
                  </c:url>

                  <tr bgcolor="#ffffff">
                    <td align="left" height="1" nowrap="nowrap">
                      <ucf:link href="#" onclick="showPopup('${ah3:escapeJs(viewUrlId)}',800,600);return false;" label="${ub:i18n(script.name)}" enabled="${canEdit}"/>
                    </td>
                    ...
                    <td>
                      ...
                      <c:forEach var="workflowDefName" items="${libWorkflowDefNameList}" varStatus="status">
                        <c:if test="${status.first}"><span class="bold">${ub:i18n("ScriptWorkflows")}</span> </c:if> ${fn:escapeXml(workflowDefName)} <c:if test="${!status.last}"> | </c:if>
                      </c:forEach>
                    </td>
                    ...
                    <td align="center" height="1" nowrap="nowrap"  width="10%">
                        <ucf:link href="#" onclick="showPopup('${ah3:escapeJs(viewUrlId)}',800,600);return false;"
                            label="${ub:i18n('View')}" img="${iconMagnifyGlass}"
                            enabled="${canEdit}"/>&nbsp;
                        ...
                        <ucf:deletelink href="${deleteUrlId}" label="${ub:i18n('Delete')}" name="${ub:i18n(script.name)}" img="${iconDeleteUrl}"
                             enabled="${script.deletable && canEdit && empty projectNameList && empty workflowDefNameList}"/>
                    </td>
                  </tr>
                </c:forEach>
              </c:otherwise>
