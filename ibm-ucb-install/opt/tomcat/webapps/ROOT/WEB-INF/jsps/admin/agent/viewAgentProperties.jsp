...
<%
  Agent agent = (Agent) pageContext.findAttribute(WebConstants.AGENT);

  pageContext.setAttribute("hasWritePermission", Boolean.valueOf(agent.isUserEditable()));

  String errorMsg = (String) pageContext.getAttribute("connectionErrorMessage");
...
<%-- TABS --%>
  ...
  <jsp:include page="/WEB-INF/jsps/admin/agent/agentTabs.jsp">
     <jsp:param name="selected" value="AgentProperties"/>
     <jsp:param name="disabled" value="${inEditMode}"/>
  </jsp:include>
...
<%-- CONTENT--%>
  ...
  <div class="contents">
    <div class="system-helpbox">${ub:i18n("AgentPropertySystemHelpBox")}</div>

    <%@include file="agentPropertyTable.inc"%>

    <br/>

    <c:if test="${inEditMode}">
        <%@include file="editAgentPropertyForm.inc"%>
    </c:if>
