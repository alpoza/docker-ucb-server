...
<%@page import="com.urbancode.ubuild.web.admin.script.step.*" %>
...
<c:if test="${param.enabled}">
  <script type="text/javascript"><!--
    function newStepPreConditionScript() {
      openStepPreConfitionScriptPopup('<%=StepPreConditionScriptPopup.methodUrl("newStepPreConditionScript", false)%>');
    }

    function editStepPreConditionScript() {
      ...
      var url = '<%=StepPreConditionScriptPopup.methodUrl("editStepPreConditionScript", false)%>';
      if (index != null) {
        if (url.indexOf('?')==-1){
          url+='?';
        }
      ...
      openStepPreConfitionScriptPopup(url);
    }

    function openStepPreConfitionScriptPopup(url) {
      openPopup(url, '_blank', 820, 520);
    }
    //-->
  </script>
  <c:if test="${canCreateScript}">&nbsp;<a href="javascript:newStepPreConditionScript();">${ub:i18n("NewScript")}</a></c:if>
  <c:if test="${canCreateScript and canEditScript}">&nbsp;&nbsp;/&nbsp;&nbsp;</c:if>
  <c:if test="${canEditScript}"><a href="javascript:editStepPreConditionScript();">${ub:i18n("EditScript")}</a>&nbsp;</c:if>
</c:if>
