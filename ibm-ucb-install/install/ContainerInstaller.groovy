...
public class ContainerInstaller {
    ...
    private String getHeapOnOOMString(javaHome, classpath) {
        String vend = getJavaVendor(javaHome, classpath);
        String result = "";
        if (vend == "ibm") {
            result = "-Xdump:heap:none " +
            "-Xdump:system:events=systhrow,filter=java/lang/OutOfMemoryError,priority=999 -Xdump:java:events=user,priority=100"
        }
        else {
            result = "-XX:+HeapDumpOnOutOfMemoryError";
       }
       return result;
    }
    ...
    private void copyServerFilesUnix(installServerDir) {
        ...
        def serverJavaOpts = "-Xmx1024m -XX:MaxPermSize=192m " +
                ...
                getHeapOnOOMString(javaHome, classpath) + " " +
    ...
    private void generateKeyManually(File keystoreLocation) {
        ...
        Certificate cert = keyHelper.buildWithExtensions("CN=localhost", from, to,
