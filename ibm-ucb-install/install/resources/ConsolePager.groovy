...
public class ConsolePager {
    ...
    protected pause(String prefix) {
        ...
        if (java6) {
            String inputText = System.console()?.readLine(prefix+" \"Enter\" for the next page, or \"F\" to print the full license: ");
            if (inputText.toLowerCase().startsWith("f")) {
                rows = 100000;
            }
        }
        else {
            println prefix+" \"Enter\" for the next page, or \"A\" to print the full license: "
            def inputText = new LineNumberReader(new InputStreamReader(System.in)).readLine();
            if (inputText.toLowerCase().startsWith("f")) {
                rows = 100000;
            }
        }
