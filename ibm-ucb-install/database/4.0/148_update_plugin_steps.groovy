...
sql.eachRow("SELECT * FROM JOB_CONFIG_STEP WHERE PLUGIN_COMMAND_ID IS NOT NULL") { row ->
    def id = row["ID"]
    def doc = parseDocument(row["DATA"])
    def docElement = doc.documentElement
    def propSheetId = getFirstChildElementText(docElement, 'prop-sheet-id')
    sql.executeUpdate("UPDATE JOB_CONFIG_STEP SET PROP_SHEET_ID = ? WHERE ID = ?", [propSheetId, id])
}
