...
final def replaceMillisecondsWithSeconds = { String xml, String tagName ->
    int start = xml.indexOf('<' + tagName + '>')
    if (start >= 0) {
        int end = xml.indexOf('</' + tagName + '>')
        long mergePeriodMillis = Long.valueOf(xml.substring(start + tagName.length() + 2, end))
        long mergePeriodSeconds = mergePeriodMillis / 1000
        return xml.substring(0, start + tagName.length() + 2) + mergePeriodSeconds + xml.substring(end)
    }
    return xml
}
...
sql.eachRow("SELECT ID, DATA FROM PROJECT_TEMPLATE") { row ->
    def id = row['ID']
    String xml = documentAsString(row["DATA"])
    xml = replaceMillisecondsWithSeconds(xml, 'merge-period')
    xml = replaceMillisecondsWithSeconds(xml, 'quiet-period')
    sql.executeUpdate("UPDATE PROJECT_TEMPLATE SET DATA = ? WHERE ID = ?", [xml, id])
}