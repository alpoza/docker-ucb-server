...
def reportSql = "SELECT CR.ID, CR.NAME, BL.ID AS BUILD_LIFE_ID, CR.COV_TYPE, CR.LINE_PERC, CR.METHOD_PERC, CR.BRANCH_PERC, BL.START_DATE AS REPORT_DATE " +
    "FROM COVERAGE_REPORT CR " +
    "JOIN JOB_TRACE JT ON JT.ID = CR.JOB_TRACE_ID " +
...
sql.eachRow(reportSql) { row ->
    ...
    def type = row["COV_TYPE"]
    def linePerc = row["LINE_PERC"]
    def methodPerc = row["METHOD_PERC"]
    def branchPerc = row["BRANCH_PERC"]
    ...
    sql.executeUpdate("INSERT INTO CODE_COVERAGE_REPORT_DIM (ID, REPORT_ID, REPORT_NAME, BUILD_LIFE, COVERAGE_TYPE, LINE_PERC, " +
        "METHOD_PERC, BRANCH_PERC, REPORT_DATE, REPORT_TIME) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [reportDimId, reportName, name, buildLifeDimId, type, linePerc, methodPerc, branchPerc, reportDateDimId, reportTimeDimId])
}

def groupSql = "SELECT CG.COVERAGE_REPORT_ID, CG.NAME, CG.LINE_PERC, CG.METHOD_PERC, CG.BRANCH_PERC, CG.COMPLEXITY " +
    "FROM COVERAGE_GROUP CG " +
    "JOIN COVERAGE_REPORT CR ON CR.ID = CG.COVERAGE_REPORT_ID"

sql.eachRow(groupSql) { row ->
    def reportId = row["COVERAGE_REPORT_ID"]
    def name = row["NAME"]
    def linePerc = row["LINE_PERC"]
    def methodPerc = row["METHOD_PERC"]
    def branchPerc = row["BRANCH_PERC"]
    def complexity = row["COMPLEXITY"]
    def reportDimId = reportMap.get(reportId)
    
    sql.executeUpdate("INSERT INTO CODE_COVERAGE_GROUP (REPORT, NAME, LINE_PERC, METHOD_PERC, BRANCH_PERC, COMPLEXITY) " +
        "VALUES (?, ?, ?, ?, ?, ?)",
        [reportDimId, name, linePerc, methodPerc, branchPerc, complexity])
}
